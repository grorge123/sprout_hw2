...
void DrawWhiteSpace(int a_x, int a_y, int b_x, int b_y){ // To clean a certain space in the terminal
    for(int i = a_x; i < b_x; i++){
        for(int j = a_y; j < b_y; j++) gotoxy(i,j); printf(" ");
    }
}
...
class SpaceShip{
    ...
    void DrawSpaceShipInfo(){ // Displays HP and energy points, I aligned them with the labels printed in DrawGameLimits
        gotoxy(5, 1); printf("     ");
        for(int i = 0; i < hp; i++) gotoxy(5 + i, 1); printf("%c", 3);
        gotoxy(23, 1); printf("     ");
        for(int i = 0; i < energy; i++) gotoxy(23 + i, 1); printf("%c", 222);
    }
    ...
    void Explosion(){ // When you lose a heart :c
        ...
        gotoxy(x,y + 2); printf("* * *");
        Sleep(100);
        if(hp > 0) energy = 5; // If you still have a heart or more
        else imDead = true; // If you don't
    }
...
int main(){
    ...
    for(int i = 0; i < 10; i++){ //  Pick as many asteroids as you want. They are randomly placed in the map but not too low
    ...
    while(!ss.isDead() && score != 100){ // If you die or reach 100 points the game ends
        ...
        for(asteroid = Asteroids.begin(); asteroid != Asteroids.end(); asteroid++){ // Every asteroid checks if the spaceship shares it's coordinates :3
